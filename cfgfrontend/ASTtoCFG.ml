(* Cours "Sémantique et Application à la Vérification de programmes"
   ...
   Converts an abstract syntax tree to a control-flow-graph. CFG arcs use a
   simpler language. The conversion takes care of splitting complex
   statements and expressions, and introducing temporaries if necessary.

   Antoine Miné 2015 Ecole normale supérieure, Paris, France / CNRS / INRIA

   Modified and adapted by

   Caterina Urban 2023 Inria & École Normale Supérieure, France *)
...
(* Add a sequence of instructions to the CFG. The entry of the first
   instruction is the given node; other nodes are created. The exit node of
   the last instruction is returned. *)
...
(* Also add a sequence of instruction to the CFG. The exist of the first
   instruction is given node. The entry of the last instruction is
   returned. *)
...
(* We need to remember a lot of information during translation, such as the
   ...
   in which variable to store a returned value, etc. For gotos, arcs are
   generated at the end of the translation of each procedure, to handle more
   easily backward gotos; hence, we must also remember label and goto
   instructions for this later pass. Everything needed is wrapped in an
   env. *)
type env =
...
let rec int_expr (env : env) (expr : AbstractSyntaxTree.int_expr) :
  ...
  | AST_int_identifier (id, x) ->
      let var =
        ...
        with Not_found ->
          failwith
            (Printf.sprintf "unknown variable %s at %s" id
               (string_of_extent x) )
      in
      (env, [], CFG_int_var var)
  | AST_int_const (i, x) ->
      ...
      | None ->
          failwith
            (Printf.sprintf "function %s has no return value at %s" id
               (string_of_extent x) )
      | Some var ->
          (* we must create a temporary to hold the returned value (consider
             the case where the function is called twice in the
             expression) *)
...
and call (env : env) ((id, x) : id ext)
    (exprs : AbstractSyntaxTree.int_expr ext list) :
  ...
  let rec doargs env inst args =
    ...
    | _ ->
        failwith
          (Printf.sprintf "wrong number of arguments for function %s at %s"
             id (string_of_extent x) )
  in
...
(* Variable declarations.
   ...
   Create the variable structure, remember it in the environment, and
   translate initialization into assignments. *)
    ...
    (fun (env, inst) ((id, x), len, init) ->
      ...
      | Some (expr, x3) -> (
        ...
        | AST_arr_init arrexpr ->
            ...
            in
            ( env2
            , before @ [(CFG_arr_assign (var, CFG_arr_int es), x3)] @ inst )
        ) )
    (env, []) l

(* Translate a statement.

   Translation creates a subgraph. The first instruction of the subgraph is
   connected to the given entry node, and the last is connected to the given
   exit node. *)
...
let rec stat (env : env) (entry : node) (exit : node) (s : stat) : env =
  ...
  | AST_return (Some (expr, x)) ->
      (* return expr is translated as return = expr the assignment is
         connected directly to the function exit *)
  ...
  | AST_if ((expr, _), (s1, x1), Some (s2, x2)) ->
      (* /--[expr]---> node_t --[s1]--\ entry --[before]--> entry1 --| |--->
         exit \--[!expr]--> node_f --[s2]--/ *)
      let env1, before, e = bool_expr env expr in
      (* entry --[before]--> entry1 *)
      let entry1 = append_inst entry before in
  ...
  | AST_while ((expr, _), (s1, x1)) ->
      (* similar to "if expr then s1", except that we have node_t --[s1]-->
         entry instead of node_t --[s1]--> exit *)
      let env1, before, e = bool_expr env expr in
      (* entry --[before]--> entry1 *)
      let entry1 = append_inst entry before in
      let node_t = create_node (fst x1) in
  ...
  | AST_for (init, expr, incr, (s1, x1)) ->
      ...
      in
      (* conditional *)
      (* head --[before]--> head1 ---[expr]---> node_t \--[!expr]--> exit *)
      let env2, before, e =
        match expr with
  ...
  | AST_label (id, x) ->
      ...
      (* failwith (Printf.sprintf "duplicate label %s at %s" id
         (string_of_extent x)); *)
      add_arc entry exit (CFG_label id) ;
      {env with env_labels= StringMap.add id entry env.env_labels}
  | AST_goto (id, x) ->
      (* remember the goto; we will generate at the end of the function, when
         all the labels are known *)
      {env with env_gotos= (entry, (id, x)) :: env.env_gotos}

(* Translate a sequence of statements. *)

and stat_list (env : env) (entry : node) (exit : node) (l : stat ext list) :
    env =
  match l with
  | [] ->
      (* entry --[skip]--> exit *)
...
let func ?(is_main = false) (env : env) (f : fun_decl) : env =
  ...
  let args =
    List.map (fun ((t, _), (id, x)) -> create_var id x t) f.fun_args
  in
  let ret =
  ...
  List.iter
    ...
    env4.env_gotos ;
  (* create function to convert arbirary properties that are valid inside the
     main function *)
...
let prog ((t, x) : toplevel list ext) (main : string) :
    cfg * (AbstractSyntaxTree.bool_expr -> ControlFlowGraph.bool_expr) =
  (* initial environment *)
  arcs := [] ;
  nodes := [] ;
  let env_init =
