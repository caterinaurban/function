(* Cours "Sémantique et Application à la Vérification de programmes"
   ...
   Samuel Marco Ueltschi 2017 ETH Zurich, Switzerland
...
(* Many parts of the syntax are tagged with an extent indicating which part
   of the parser-file corresponds to the sub-tree. This is very useful for
   interesting error reporting! *)
type 'a ext = 'a * extent
...
type stat =
  ...
  (* declaration of a local variable, live until the end of the current
     block *)
...
and loc_decl = typ ext * (* type *) loc_init list
(* variable list *)
...
and loc_init =
  ...
  * (* declared variable *)
  int_expr ext option
  * (* length for arrays *)
  init_expr ext option
(* initializer *)

(* function declaration (no return type, all functions return void) *)
type fun_decl =
  { (* function name *)
    fun_name: id ext
