...
(* Control-flow graphs (CFG).
   ...
   variables are desambiguated; - the binding of formal and actual arguments
   is explicit (as assignments); - the control structures are translated into
   jumps between nodes. *)
...
(* Each global variable, local variable and function parameter is associated
   a var structure. We use unique identifiers (integers) to distinguish
   between variables declared at different point with the same name. *)
type var =
...
(* Expressions in the CFG are call-free. Calls are extracted during the
   translation form AST to CFG and put in separate instructions (possibly
...
(* Each arc between two CFG node is labelled with an instruction to execute
   to go from the source node to the destination node. CFG instructions are
   thus very simple. *)

type inst =
  (* go to the destination node doing nothing *)
  (* the string argument is just for printing, it give some information on
     the original instruction that caused the skip *)
  | CFG_skip of string
...
(* Functions have a single entry node and a single exit node. The execution
   always starts at the entry node, and the function always returns through
   the return node.

   A return instruction inside the function is compiled as a jump to the exit
   node. Any returned value is stored into a special variable before jumping
   to the exit node. *)
and func =
...
(* Each CFG node is denoted by a unique (integer) identifier. A CFG node
   corresponds roughly to a position in the program, but complex statements
   and expressions can be split among many nodes. *)
and node =
...
type cfg =
  ...
  ; cfg_init_entry: node
        (* first node of code initializing global variables *)
...
(* check if all arcs in cfg have corresponding entry in node_in and
   node_out *)
...
(* Insert 'exit' label befor exit node and 'entry' label before entry node in
   'func' *)
...
(* Replace each function call edge in cfg with an edge from the original call
   source entering the function and adds one returning back from the function
   exit to the destination of the original call edge. *)
let add_function_call_arcs (cfg : cfg) =
  ...
  let add_function_call_arc (cfg : cfg) (arc : arc) =
    ...
    | CFG_call f ->
        (* replace function call arc with arc from source node to function
           entry *)
        let src_func_arc =
          { arc_id= arc.arc_id
          ; arc_src= arc.arc_src
...
let func_nodes (func : func) =
  let rec aux nodeMap (node : node) =
    if NodeMap.mem node nodeMap then nodeMap
      ...
      List.fold_left aux nodeMap'
        (List.map (fun a -> a.arc_dst) node.node_out)
  in
...
let copy_function_nodes (cfg : cfg) (func : func) =
  ...
  List.iter
    (fun arc ->
      ...
      in
      let replaceWithNewArc a =
        if a.arc_id == arc.arc_id then newArc else a
      in
...
let inline_function_calls (cfg : cfg) =
  ...
  let inlineCallArc canInline cfg call_arc =
    ...
    if canInline calledFunction then
      (* copy all nodes in the function to inline, replace the call arc with
         an edge to the entry of the copied function and add add back edge
         from the exit. *)
  ...
  in
  (* Keep inlining all functions that don't call any other functions until
     only recursive functions are left *)
  let rec aux cfg =
    ...
    (* keep going until there are no more function calls that can be
       inlined *)
  in
  aux cfg
