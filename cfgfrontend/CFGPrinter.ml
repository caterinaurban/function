...
let print_option f none ch l =
  match l with None -> output_string ch none | Some v -> f ch v
...
let rec print_int_expr ch e =
  ...
  | CFG_int_unary (op, e1) ->
      ...
      if int_expr_precedence e1 <= int_expr_precedence e then
        Printf.fprintf ch " (%a)" print_int_expr e1
  ...
  | CFG_int_binary (op, e1, e2) ->
      ...
      if int_expr_precedence e2 <= int_expr_precedence e then
        Printf.fprintf ch " (%a)" print_int_expr e2
  ...
  | CFG_arr_elm (v, e) ->
      Printf.fprintf ch "%a[%a]" print_var v print_int_expr e

and print_arr_expr ch e =
  match e with
  | CFG_arr_int es ->
      Printf.fprintf ch "{" ;
      List.iter (fun e -> Printf.fprintf ch "  %a, " print_int_expr e) es ;
      Printf.fprintf ch "}"
...
and print_bool_expr ch e =
  ...
  | CFG_bool_unary (op, e1) ->
      ...
      if bool_expr_precedence e1 <= bool_expr_precedence e then
        Printf.fprintf ch " (%a)" print_bool_expr e1
  ...
  | CFG_bool_binary (op, e1, e2) ->
      ...
      if bool_expr_precedence e2 <= bool_expr_precedence e then
        Printf.fprintf ch " (%a)" print_bool_expr e2
...
let print_inst ch i =
  ...
  | CFG_elm_assign (v, i, e) ->
      Printf.fprintf ch "%a[%a] = %a" print_var v print_int_expr i
        print_int_expr e
  | CFG_arr_assign (v, e) ->
      Printf.fprintf ch "%a[] = %a" print_var v print_arr_expr e
...
let print_cfg ch p =
  let pp_var ch v =
    Printf.fprintf ch "%s:%s" (unique_var_name v) (string_of_type v.var_type)
  in
  ...
  List.iter
    (fun f ->
      Printf.fprintf ch
        "  %i: %s(%a) -> %a at %s, entry: %i, exit: %i, calls:" f.func_id
        f.func_name (print_list pp_var ",") f.func_args
  ...
  List.iter
    (fun n ->
      Printf.fprintf ch "  %i: at %s, in: " n.node_id
        (string_of_position n.node_pos) ;
      List.iter
        (fun a -> Printf.fprintf ch "%i " a.arc_src.node_id)
        n.node_in ;
      Printf.fprintf ch "out:" ;
      List.iter
        (fun a -> Printf.fprintf ch "%i " a.arc_dst.node_id)
        n.node_out ;
      Printf.fprintf ch "\n" )
    p.cfg_nodes ;
  Printf.fprintf ch "\n" ;
