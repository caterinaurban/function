(* ********* Abstract Syntax ************ - no side-effects - labeled
   statements Copyright (C) 2012-2014 by Caterina Urban. All rights
   reserved. *)
...
type arrayvar = {arrName: string; arrId: string; size: int; vars: var list}
...
let rec aExp_to_apron e =
  ...
  | A_abinary (o, (e1, _), (e2, _)) -> (
      ...
      | A_MULTIPLY ->
          Texpr1.Binop (Texpr1.Mul, e1, e2, Texpr1.Int, Texpr1.Zero)
...
let rec negBExp (b, a) =
  ...
  | A_bbinary (o, b1, b2) ->
      (A_bbinary (negBBinOp o, negBExp b1, negBExp b2), a)
...
let property_print fmt p =
  ...
  if StringMap.cardinal p > 0 then
    StringMap.iter
      (fun l e -> Format.fprintf fmt "%s -> %a\n" l bExp_print e)
      p
...
and parameter_print fmt (s, _) =
  match s with
  | A_assign (v, e) ->
      Format.fprintf fmt "%a := %a" aExp_print v aExp_print e
...
and label_print fmt l =
  if l < 10 then Format.fprintf fmt "[ %i:]" l
  else Format.fprintf fmt "[%i:]" l
