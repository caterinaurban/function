...
(* types *)
let typ_itoa ((t : IntermediateSyntax.typ), (_ : extent)) :
    AbstractSyntax.typ =
  match t with I_INT -> A_INT

(* expressions *)
let rec exp_itoa (ctx : ctx) (* calling context *)
    (env : env) (* environment *)
    ...
    ((exp : IntermediateSyntax.exp), (a : extent)) :
  ...
  try
    ...
    | I_const i ->
        (A_arithmetic (A_const (int_of_string i), a), env, pre, post)
    ...
    | I_mod _ ->
        raise Modulo (* modulo is approximated with non-determinism *)
    | I_less (e1, e2) | I_leq (e1, e2) | I_greater (e1, e2) | I_geq (e1, e2)
      -> (
        ...
        | _ ->
            raise (Invalid_argument "exp_itoa:I_less/I_leq/I_greater/I_geq")
        )
    | I_eq (e1, e2) -> (
        let e1, env, pre, post = exp_itoa ctx env pre post e1 in
        let e2, env, pre, post = exp_itoa ctx env pre post e2 in
        ...
        | A_arithmetic (e1, ea1), A_arithmetic (e2, ea2) -> (
          ...
          | _ ->
              let lhs =
                (A_rbinary (A_LESS_EQUAL, (e1, ea1), (e2, ea2)), a)
              in
              let rhs =
...
(** Converts a parsed property to it's AbstractSyntax version. In constrast
    to 'property_itoa', this function takes the result of 'prog_itoa' and a
    main function as argument. This makes it possible to convert additional
    properties for a given program without having to construct the internal
    context and environment data structures after the program has already
    been converted. *)
...
let declarator_itoa ctx (* ctx *) gs (* var StringMap.t *) ls
  ...
  let env =
    ...
    else {globals= vm; locals= ls; funcs= fs; return= None; constants= cs}
...
let globalDecl_itoa ctx (* ctx *) env (* env *) scope stmts
    (* Asyntax.stmt list *)
      ( (typ (* Isyntax.typ annotated *), decls (* Isyntax.declarator list *))
      , _ ) =
  let vmss =
...
let rec stmt_itoa (ctx : ctx) (* calling context *)
    (env : env) (* environment *)
    ((stmt : IntermediateSyntax.stmt), (a : extent)) : statements * env =
  (* annotated statement *)
  match stmt with
  ...
  | I_if (exp, stmt1, stmt2) -> (
      ...
      | Some stmt2 -> (
          ...
          | A_arithmetic (e, ea) ->
              let lhs = (A_rbinary (A_LESS, (e, ea), (A_const 0, ea)), ea) in
              let rhs =
                (A_rbinary (A_GREATER, (e, ea), (A_const 0, ea)), ea)
              in
