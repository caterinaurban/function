(* ********* (Linear) Constraints ************ Copyright (C) 2012-2014 by
...
module type CONSTRAINT = sig
  ...
  val similar :
    t -> t -> bool (* constraints differing only for the constant *)
...
module Constraint : CONSTRAINT = struct
  ...
  let compare c1 c2 =
    let rec aux l1 l2 =
      ...
      | [], [] ->
          ...
          if c = 0 then
            compareTyp (Lincons1.get_typ c1) (Lincons1.get_typ c2)
          else c
      | [], (x2, v2) :: l2s ->
  ...
  (* let variant c1 (* a_1x_1 + ... + a_nx_n >= a_n+1 *) c2 (* b_1x_1 + ... +
     b_nx_n >= b_n+1 *) = let rec aux l1 l2 = match l1,l2 with | [],[] -> let
     v1 = Lincons1.get_cst c1 in let v2 = Lincons1.get_cst c2 in let p1 =
     compareCoeff v1 (Coeff.s_of_int 0) in let p2 = compareCoeff v2
     (Coeff.s_of_int 0) in if (p1 > 0) && (p2 > 0) then compareCoeff v1 v2 <=
     0 else if (p1 < 0) && (p2 < 0) then compareCoeff v1 v2 >= 0 else (p1 =
     0) && (p2 = 0) | (x1,v1)::l1s,(x2,v2)::l2s when (Var.compare x1 x2) = 0
     -> let p1 = compareCoeff v1 (Coeff.s_of_int 0) in let p2 = compareCoeff
     v2 (Coeff.s_of_int 0) in if (p1 > 0) && (p2 > 0) then begin let c =
     compareCoeff v1 v2 in if (c <= 0) then aux l1s l2s else false end else
     if (p1 < 0) && (p2 < 0) then begin let c = compareCoeff v1 v2 in if (c
     >= 0) then aux l1s l2s else false end else if (p1 = 0) && (p2 = 0) then
     aux l1s l2s else false | _ -> false in let t1 = Lincons1.get_typ c1 and
     t2 = Lincons1.get_typ c2 in match t1,t2 with |
     Lincons1.SUPEQ,Lincons1.SUPEQ when 0 = compareCoeff (Lincons1.get_cst
     c1) (Lincons1.get_cst c2) -> let l1 = ref [] and l2 = ref [] in
     Lincons1.iter (fun v x -> l1 := (x,v)::!l1) c1; Lincons1.iter (fun v x
     -> l2 := (x,v)::!l2) c2; aux !l1 !l2 | _ -> false *)
  ...
  (* let widen c1 (* a_1x_1 + ... + a_nx_n >= a_n+1 *) c2 (* b_1x_1 + ... +
     ...
     b_n+1-a_n+1 *) let env = Lincons1.get_env c1 in let c = Lincons1.make
     (Linexpr1.make env) (Lincons1.get_typ c1) in let rec aux l1 l2 = match
     l1,l2 with | [],[] -> let v1 = Coeff.neg (Lincons1.get_cst c1) in let v2
     = Lincons1.get_cst c2 in Lincons1.set_cst c (addCoeff v1 v2) (*
     Lincons1.set_cst c (Coeff.s_of_int 0) *) | (x1,v1)::l1s,(x2,v2)::l2s
     when (Var.compare x1 x2) = 0 -> Lincons1.set_coeff c x1 (addCoeff
     (Coeff.neg v1) v2); aux l1s l2s | _ -> raise (Invalid_argument
     "widen:aux:") in let l1 = ref [] and l2 = ref [] in Lincons1.iter (fun v
     x -> l1 := (x,v)::!l1) c1; Lincons1.iter (fun v x -> l2 := (x,v)::!l2)
     c2; aux !l1 !l2; c *)
