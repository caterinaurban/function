...
(* OCaml - commit 0aa72dd0340fb51dbd412a39440281e4cdde97fd with additions by
   Antoine Mine' (C) 2011 *)
...
module Make (Ord : OrderedType) : S with type key = Ord.t = struct
  ...
  (* Beware: those two functions assume that the added k is *strictly*
     smaller (or bigger) than all the present keys in the tree; it does not
     test for equality with the current min (or max) key.

     Indeed, they are only used during the "join" operation which respects
     this precondition. *)
  ...
  let rec seq_of_enum_ c () =
    ...
    | End -> Seq.Nil
    | More (k, v, t, rest) ->
        Seq.Cons ((k, v), seq_of_enum_ (cons_enum t rest))
  ...
  (* as above, but ignore physically equal subtrees - for map, assumes: f k d
     d = d - for iter, assumes: f k d d has no effect - for fold, assumes: k
     f d d acc = acc - for for_all, assumes: f k d d = true - for exists,
     assumes: f k d d = false *)
