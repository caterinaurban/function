(* Linearization functions. Copyright (C) 2011 Antoine Miné *)
...
module Make (I : INTERVAL) = struct
  ...
  (* constraints have the for "expr op 0"; we also remember whether the
     constraint is integer, for subsequent negations *)
  type cons_op =
    | L_EQ (* = 0 *)
    | L_NEQ (* != 0 *)
    | L_GEQ (* >= 0 *)
  ...
  let rec lin_expr (env : env) ((e, t, x) : expr typed) : linexpr =
    ...
    | T_binary (op, e1, e2) -> (
        ...
        | A_DIVIDE -> (
          (* division creates reals from ints, hence the need for a cast from
             real *)
  ...
  (* constraint linearization returns both the true and false conditions *)
  ...
  (* quasi = false: interval variable coefficients allowed quasi = true: only
     scalar variable coefficients allowed *)
  ...
  (* inverts the assignment, which should be quasi-linear, with non-nul
     coefficient for v; the result can be made quasi-linear *)
  ...
  let split_cons ((l, op) : lincons) : lincons * lincons =
    ...
    | _ ->
        invalid_arg "split_cons: not an equality or disequality constraint"
