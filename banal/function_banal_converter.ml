...
let rec of_aExp (aExp:FAS.aExp) : expr = 
  match aExp with 
  ...
  | FAS.A_interval (i1,i2) ->
    ...
    T_int_const (i1, i2)
  | FAS.A_aunary (op, (e,_)) ->
    let expr = (of_aExp e, int_type, dummy_extent) in
    let unOp = match op with | FAS.A_UMINUS -> A_UNARY_MINUS in
    T_unary (unOp, expr)
  | FAS.A_abinary (op, (e1,_), (e2,_)) ->
    let expr1 = (of_aExp e1, int_type, dummy_extent) in
    let expr2= (of_aExp e2, int_type, dummy_extent) in
...
let rec of_bExp (bExp:FAS.bExp) : expr = 
  match bExp with 
  | FAS.A_TRUE -> T_bool_const True
  | FAS.A_MAYBE	-> T_bool_const Maybe
  | FAS.A_FALSE -> T_bool_const False
  | FAS.A_bunary (op, (e,_)) ->
    let expr = (of_bExp e, A_BOOL, dummy_extent) in
    let uOp = match op with | FAS.A_NOT -> A_NOT in
    T_unary (uOp, expr)
  | FAS.A_bbinary (op, (e1,_), (e2,_)) ->
    let expr1 = (of_bExp e1, A_BOOL, dummy_extent) in
    let expr2 = (of_bExp e2, A_BOOL, dummy_extent) in
