...
type 't abstract_transformer =
  ...
  -> (* current value of abstract state for this node *)
     ...
     (** list of (abstract state, instruction) pairs for all predecessors
         (successor) nodes that have an incoming (outgoing) edge from (to)
         this node. *)
  -> bool * 't
(* returns a boolean flag that indicates if a fixed point was reached for
   this node and the updated state for this node *)
...
(* abstract implementation of a backward/forward analysis over a control flow
   graph *)
let execute (type a)
    ...
    (* returns a map that assigns each node an abstract state that is the
  ...
  (* add all predecessors of 'node' to worklist if they are not already in
     it*)
    ...
    List.iter
      (fun arc ->
        ...
        if not inWorklist.(predecessor.node_id) then
          (* if the predecessor is a loop head we do NOT add it to the
             worklist if the worklist still contains nodes that are dominated
             by it *)
          let unfinished =
  ...
  let getOutgoingStates nodeMap node =
    List.map
      ...
      node.node_out
  in
  (* choose which states are passed to abstract transformer based on analysis
     type *)
  ...
  in
  (* array that counts the number of times a node as heen processed NOTE:
     this assumes that nodes have ids numbered from 1...n and that the array
     is accessed through this id e.g. processed.(i) where 'i' is the node
     id *)
  let processed = Array.make (nodeCount + 1) 0 in
  (* auxiliary function that implements the worklist algorithm expressed in
     tail-recursive form. Takes an inital 'nodeMap' as argument that assigns
     an abstract state to each node and returns a final 'nodeMap' containing
  ...
  let rec aux (nodeMap : a NodeMap.t) : a NodeMap.t =
    ...
    if Queue.is_empty worklist then nodeMap (* Work list is empty => exit *)
      ...
      (* add predecessors (successors) of node to worklist if either we
         didn't reach a fixed point or if this is the first time we process
         this node *)
...
let forward_analysis (type a) (at : a abstract_transformer) =
  execute FORWARD at
