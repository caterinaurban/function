...
let ( => ) file result =
  TestCommon.testit ~joinbwd:2 ~analysis_type:"termination" ~retrybwd:5
    ~dom:POLYHEDRA file
  = result
...
let ( *=> ) file result =
  TestCommon.testit ~joinbwd:3 ~analysis_type:"termination" ~dom:POLYHEDRA
    ~ord:(true, 2) file
  = result

(* costly analyzer configurations needed for few programs *)
let ( +=> ) file result =
  TestCommon.testit ~joinbwd:5 ~analysis_type:"termination" ~dom:POLYHEDRA
    ~retrybwd:5 file
  = result

let ( ^=> ) file result =
  TestCommon.testit ~joinbwd:7 ~analysis_type:"termination" ~dom:POLYHEDRA
    ~retrybwd:5 file
  = result

let ( $=> ) file result =
  TestCommon.testit ~joinbwd:2 ~analysis_type:"termination" ~dom:POLYHEDRA
    ~retrybwd:5 file
  = result (* conflict-driven learning *)

let ( @=> ) file result =
  TestCommon.testit ~joinbwd:2 ~analysis_type:"ctl" ~dom:POLYHEDRA
    ~prop:"AF{exit:true}" ~retrybwd:5 file
  = result

let ( @!=> ) file result =
  TestCommon.testit ~joinbwd:2 ~ord:(true, 2) ~analysis_type:"ctl"
    ~dom:POLYHEDRA ~prop:"AF{exit:true}" ~retrybwd:5 file
  = result

let ( @@=> ) (filename, conditionalTerminationProperty) result =
  TestCommon.testit ~joinbwd:2 ~analysis_type:"ctl" ~prop:"AF{exit:true}"
    ~retrybwd:5 ~precond:conditionalTerminationProperty ~dom:POLYHEDRA
    filename
  = result
...
let%test "sas2014b_ctl" = "termination/sas2014b.c" @=> true

let%test "sas2014c_ctl" = "termination/sas2014c.c" @=> false
(* conditionally terminating *)

let%test "sorting4_ctl" = "termination/sorting4.c" @=> true

let%test "tacas2013_ctl" = "termination/tacas2013a.c" @=> true

let%test "tacas2013b_ctl" = "termination/tacas2013b.c" @=> true
...
let%test "euclid_ctl precond" = ("termination/euclid.c", "x==y") @@=> true
...
let%test "issue8 precond" =
  ("termination/issue8.c", "y >= z || x > 0 || -y >= 2") @@=> true
